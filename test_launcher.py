...
BASE_DIR = get_exe_real_path()

OK_CMD = os.path.join(BASE_DIR, "ok.exe")
OK_SCRIPT_DIR = os.path.join(BASE_DIR, "my_game")
...
def start_ok_script():
    ...
    print(f"OK_CMD = {OK_CMD}")
    print(f"OK_CMD exists = {os.path.exists(OK_CMD)}")
    print(f"OK_SCRIPT_DIR = {OK_SCRIPT_DIR}")
    print(f"OK_SCRIPT_DIR exists = {os.path.exists(OK_SCRIPT_DIR)}")
    ...
    try:
        ...
        ok_process = subprocess.Popen(
            [OK_CMD, "run_task", OK_SCRIPT_TASK, "--config", OK_SCRIPT_CONFIG],
            cwd=OK_SCRIPT_DIR,
            stdout=log_file,
            stderr=log_file,
            creationflags=subprocess.CREATE_NO_WINDOW
        )
        print("ok.exe 启动成功！pid=", ok_process.pid)
    except Exception as e:
        print(f"Popen异常: {e}")
